...
from .geometry import FactorizationMode, SampledPattern
...
@dataclass
class PatternLayer:
    """Mutable sampled anisotropic layer built in two steps.

    First create the layer with thickness/background/grid information, then
    draw one or more x-y patterns on that layer, and finally convert it to a
    solver ``Layer`` with ``toLayer()``.
    """

    period: tuple[float, float]
    thickness: float
    background: complex | float | object
    shape: tuple[int, int]
    name: str = ""
...
@dataclass
class LayerStack:
    """Layer-first 3D geometry builder for anisotropic RCWA structures."""
